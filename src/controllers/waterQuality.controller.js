...
export async function getWaterQualityReport(req, res) {
  try {
    const realEstateId = req.query.realEstateId !== undefined && req.query.realEstateId !== "" ? Number(req.query.realEstateId) : 0;
    const { from, to } = req.query;

    const data = await WaterQualityModel.getWaterQualityReport(realEstateId, from, to);
    return response.success(res, "Water quality report fetched successfully", data);
  } catch (err) {
